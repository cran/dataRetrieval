...
test_that("General USGS retrievals working", {
  ...
  dv_data <- read_waterdata(
    service = "daily",
    CQL = cql,
    time = c("2023-01-01", "2024-01-01")
  )

  expect_equal(as.Date(c("2023-01-01", "2024-01-01")), range(dv_data$time))
  expect_true(all(
    unique(dv_data$monitoring_location_id) %in%
...
test_that("General NWIS retrievals working", {
  ...
  timeseriesInfo <- read_waterdata_ts_meta(
    bbox = sf::st_bbox(siteInfo),
    parameter_code = "00010",
  ...
  ohio <- read_waterdata_monitoring_location(
    state_name = "Ohio",
    site_type_code = "ST"
  )
  bbox <- sf::st_bbox(ohio)
  what_sites <- read_waterdata_ts_meta(parameter_code = "00665", bbox = bbox)
  expect_true(all(
    c("monitoring_location_id", "begin", "end", "parameter_name") %in%
      names(what_sites)
  ))
  ...
  county_code_stafford <- countyCdLookup(
    state = "Virginia",
    county = "Stafford",
    outputType = "id"
  )
  state_code_va <- stateCdLookup(input = "Virginia", outputType = "id")
  stafford <- read_waterdata_monitoring_location(
    county_code = county_code_stafford,
  ...
  multi_huc <- read_waterdata_daily(
    bbox = sf::st_bbox(multi_huc_sites),
...
test_that("read_waterdata_ts_meta", {
  ...
  availableData <- read_waterdata_ts_meta(
    monitoring_location_id = "USGS-05114000"
  )
  expect_gte(ncol(availableData), 18)

  uvData <- read_waterdata_ts_meta(
    monitoring_location_id = "USGS-05114000",
  ...
  # multiple services
  uvDataMulti <- read_waterdata_ts_meta(
    monitoring_location_id = c("USGS-05114000", "USGS-09423350"),
    computation_period_identifier = c("Daily", "Points")
  )

  expect_true(all(
    unique(uvDataMulti$computation_period_identifier) %in% c("Daily", "Points")
  ))

  # state codes:
  wi_sites <- read_waterdata_monitoring_location(state_name = "Wisconsin")
  flow_and_temp <- read_waterdata_ts_meta(
    bbox = sf::st_bbox(wi_sites),
  ...
  expect_true(all(
    unique(flow_and_temp$computation_period_identifier) %in%
      c("Daily", "Points")
  ))
  expect_true(all(
    unique(flow_and_temp$parameter_code) %in% c("00060", "00010")
  ))
...
test_that("Dates with no days can be handled", {
  ...
  empty_df <- read_waterdata_field_measurements(
...
test_that("read_waterdata_ts_meta working", {
  ...
  siteListPhos <- read_waterdata_ts_meta(
    bbox = sf::st_bbox(siteListOhio),
    parameter_code = "00665"
  )
  expect_true(nrow(siteListPhos) > 0)
  expect_is(siteListPhos$begin, "POSIXct")

  bboxSites <- read_waterdata_ts_meta(
    bbox = c(-92.5, 45.4, -87, 47),
    parameter_code = "00060"
  )
...
test_that("format_dates", {
  ...
  start_end <- c("", "")
  expect_equal(dataRetrieval:::format_api_dates(start_end), NA)
  ...
  expect_equal(dataRetrieval:::format_api_dates(start, TRUE), "2021-01-01/..")

  end <- c(NA, "2021-01-01")

  expect_equal(dataRetrieval:::format_api_dates(end), "../2021-01-01T00:00:00Z")
  expect_equal(dataRetrieval:::format_api_dates(end, TRUE), "../2021-01-01")
  #
  ...
  start_end <- as.POSIXct(
    c("2021-01-01 12:15:00", "2022-01-01 16:45"),
    tz = "America/New_York"
  )
  ...
  expect_equal(
    dataRetrieval:::format_api_dates(c("2010-01-01T00:00Z", NA)),
    "2010-01-01T00:00:00Z/.."
  )

  expect_equal(
    dataRetrieval:::format_api_dates(c(
      "2010-01-01T00:00Z",
      "2010-01-01T05:00Z"
    )),
