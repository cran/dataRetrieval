...
#'
#' }
construct_waterdata_sample_request <- function(
  ...
  dataType <- match.arg(
    dataType,
    c("results", "locations", "activities", "projects", "organizations"),
    several.ok = FALSE
  )
  ...
  switch(
    ...
    results = {
      ...
      baseURL <- httr2::req_url_path_append(baseURL, "results")
    },
    locations = {
      available_profiles <- c("site", "count")

      baseURL <- httr2::req_url_path_append(baseURL, "locations")
    },
    activities = {
      available_profiles <- c("sampact", "actmetric", "actgroup", "count")

      baseURL <- httr2::req_url_path_append(baseURL, "activities")
    },
    projects = {
      available_profiles <- c("project", "projectmonitoringlocationweight")

      baseURL <- httr2::req_url_path_append(baseURL, "projects")
    },
    organizations = {
      available_profiles <- c("organization", "count")

      baseURL <- httr2::req_url_path_append(baseURL, "organizations")
    }
  )

  dataProfile <- check_profile(dataProfile, available_profiles)
  dataProfile <- match.arg(dataProfile, available_profiles, several.ok = FALSE)

  baseURL <- httr2::req_url_path_append(baseURL, dataProfile)

  if (all(!is.na(siteTypeCode))) {
    siteTypeCode <- match.arg(
      siteTypeCode,
  ...
  if (all(!is.na(stateFips))) {
    ...
    state_codes <- paste(states$countryCode, states$fipsCode, sep = ":")
    stateFips <- match.arg(stateFips, state_codes, several.ok = TRUE)
  }
  ...
  if (all(!is.na(countyFips))) {
    ...
    state_cd <- stats::setNames(states$fipsCode, states$stateAbbrev)
    county_codes <- paste(
    ...
    countyFips <- match.arg(countyFips, county_codes, several.ok = TRUE)
  }

  check_radius <- sum(is.na(c(
  ...
  if (!check_radius %in% c(3, 0)) {
    stop(
      "pointLocationLatitude, pointLocationLongitude, and pointLocationWithinMiles
           must all be defined, or none defined."
    )
  }

  baseURL <- explode_query(
    baseURL,
    POST = FALSE,
    list(
  ...
  if (all(!is.na(activityStartDateLower))) {
    start <- checkWQPdates(list(
      activityStartDateLower = activityStartDateLower
    ))
    start <- as.character(as.Date(
      start$activityStartDateLower,
      format = "%m-%d-%Y"
    ))
    baseURL <- httr2::req_url_query(baseURL, activityStartDateLower = start)
  }

  if (all(!is.na(activityStartDateUpper))) {
    end <- checkWQPdates(list(activityStartDateUpper = activityStartDateUpper))
    end <- as.character(as.Date(
      end$activityStartDateUpper,
      format = "%m-%d-%Y"
    ))
    baseURL <- httr2::req_url_query(baseURL, activityStartDateUpper = end)
  }
...
check_profile <- function(dataProfile, profile_convert) {
  if (is.na(dataProfile)) {
    message(paste0(
      "No profile specified, defaulting to '",
      profile_convert[1],
      "'\n",
      "Possible values are: \n",
...
explode_query <- function(baseURL, POST = FALSE, x, multi = "explode") {
  ...
  if (any(!is.na(x))) {
    ...
    if (POST) {
      baseURL <- httr2::req_body_json(req = baseURL, data = x)
    } else {
      baseURL <- httr2::req_url_query(baseURL, !!!x, .multi = multi)
    }
  }
  return(baseURL)
}
...
#' }
check_waterdata_sample_params <- function(
  service = "characteristicgroup",
  ...
) {
  service_options <- c(
    "characteristicgroup",
  ...
  if (length(list(...)) > 0) {
    params <- list(...)
    extra_params <- c("group", "pageNumber", "pageSize", "text")
    match.arg(names(params), extra_params, several.ok = TRUE)
    check_group_req <- httr2::req_url_query(check_group_req, !!!params)
  }
...
#'
#' }
read_waterdata_samples <- function(
  monitoringLocationIdentifier = NA,
  siteTypeCode = NA,
  boundingBox = NA,
  hydrologicUnit = NA,
  ...
  request_url <- construct_waterdata_sample_request(
    monitoringLocationIdentifier = monitoringLocationIdentifier,
    siteTypeCode = siteTypeCode,
    boundingBox = boundingBox,
    hydrologicUnit = hydrologicUnit,
