...
#' @param csv logical. Is the data coming back with a csv or tsv format. Default is `FALSE`.
...
#' <https://www.waterqualitydata.us/portal_userguide/> for more information.
#' @export
#' @seealso [readWQPdata()], [readWQPqw()], [whatWQPsites()]
...
#' }
#'
importWQP <- function(obs_url, tz = "UTC", csv = TRUE, convertType = TRUE) {
  if (tz != "") {
  ...
  names(retval)[grep("/", names(retval))] <- gsub(
    "/",
    ".",
    names(retval)[grep("/", names(retval))]
  )
...
parse_WQP <- function(retval, tz = "UTC") {
  ...
  for (numberCol in numberColumns) {
    suppressWarnings({
      val <- tryCatch(as.numeric(retval[[numberCol]]), warning = function(w) w)
  ...
  if (length(dateCols) > 0) {
    ...
    for (date_col in dateCols) {
      ...
      if (all(c(date_col, time_col, tz_col) %in% names(retval))) {
        ...
        all(c(date_col, time_col_wqx3, tz_col_wqx3) %in% names(retval))
      ) {
        # WQX3
        retval <- create_dateTime(
          retval,
    ...
    if (length(dateCols_to_convert) > 0) {
      for (i in dateCols_to_convert) {
        if (i %in% names(retval)) {
          retval[, i] <- suppressWarnings(as.Date(lubridate::parse_date_time(
            retval[[i]],
            c("Ymd", "mdY")
          )))
        }
...
create_dateTime <- function(df, date_col, time_col, tz_col, tz) {
  ...
  df[[date_col]] <- suppressWarnings(as.Date(lubridate::parse_date_time(
    df[[date_col]],
    c("Ymd", "mdY")
  )))
