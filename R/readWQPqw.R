...
#' ActivityStartTime.Time \tab character \tab	The time of day that is reported when
#' the field activity began, based on a 24-hour timescale.\cr
#' ActivityStartTime.TimeZoneCode \tab character \tab	The time zone for which the
#' time of day is reported.
#' Any of the longitudinal divisions of the earth's surface in which a standard time is kept.\cr
...
#' ActivityEndTime.TimeZoneCode \tab character \tab	The time zone for which the time of day is reported.
#' Any of the longitudinal divisions of the earth's surface in which a standard time is kept.\cr
#' ActivityDepthHeightMeasure.MeasureValue \tab character \tab	A measurement of the vertical location
#' (measured from a reference point) at which an activity occurred. Measure value is
#' given in the units stored in ActivityDepthHeightMeasure.MeasureUnitCode.\cr
#' ActivityDepthHeightMeasure.MeasureUnitCode \tab character \tab	The code that
#' represents the unit for measuring the item.\cr
#' ActivityDepthAltitudeReferencePointText \tab character \tab	The reference used to
#' indicate the datum or reference used to establish the depth/altitude of an activity.\cr
#' ActivityTopDepthHeightMeasure.MeasureValue \tab character \tab	A measurement of the
#' upper vertical location of a vertical location range (measured from a reference point) at which
#' an activity occurred. Measure value is given in the units stored in
...
#' represents the unit for measuring the item.\cr
#' ActivityBottomDepthHeightMeasure.MeasureValue \tab character \tab	A measurement of the lower vertical
#' location of a vertical location range (measured from a reference point) at which
#' an activity occurred. Measure value is
#' given in the units stored in ActivityBottomDepthHeightMeasure.MeasureUnitCode.\cr
...
#' MonitoringLocationIdentifier \tab character \tab	A designator used to describe the
#' unique name, number, or code assigned to identify the monitoring location.\cr
...
#' HydrologicCondition * \tab character \tab 	Hydrologic condition is the hydrologic condition that is
#' represented by the sample collected (i.e. ? normal, falling, rising, peak stage).\cr
#' HydrologicEvent * \tab character \tab 	A hydrologic event that is represented by the
#' sample collected (i.e. - storm, drought, snowmelt).\cr
#' SampleCollectionMethod.MethodIdentifier\tab character \tab 	The identification number
...
#' SampleCollectionMethod.MethodName \tab character \tab	The title that appears on the
#' method from the method publisher.\cr
...
#' CharacteristicName \tab character \tab	The object, property, or substance which is evaluated or
#' enumerated by either a direct field measurement, a direct field observation, or by
#' laboratory analysis of material collected in the field.\cr
...
#' ResultMeasureValue \tab numeric \tab	The reportable measure of the result for the chemical, microbiological
#' or other characteristic being analyzed. Measure value is given in the units stored
#' in ResultMeasure/MeasureUnitCode.\cr
...
#' ResultValueTypeName \tab character \tab	A name that qualifies the process which
#' was used in the determination of the result value (e.g., actual, estimated, calculated).\cr
#' ResultWeightBasisText \tab character \tab	The name that represents the form
#' of the sample or portion of the sample which is associated with the result value
#' (e.g., wet weight, dry weight, ash-free dry weight).\cr
...
#' ResultDepthHeightMeasure.MeasureUnitCode + \tab character \tab	The code that represents
#' the unit for measuring the item.\cr
#' ResultDepthAltitudeReferencePointText + \tab character \tab 	The reference used to
#' indicate the datum or reference used to establish the depth/altitude of a result.\cr
#' SubjectTaxonomicName \tab character \tab	The name of the organism from which a tissue sample was taken.\cr
#' SampleTissueAnatomyName  * \tab character \tab 	The name of the anatomy from
#' which a tissue sample was taken.\cr
#' ResultAnalyticalMethod.MethodIdentifier \tab character \tab	The identification
#' number or code assigned by the method publisher.\cr
#' ResultAnalyticalMethod.MethodIdentifierContext \tab character \tab	Identifies
#' the source or data system that created or defined the identifier.\cr
#' ResultAnalyticalMethod/MethodName \tab character \tab	The title that appears
#' on the method from the method publisher.\cr
...
#' DetectionQuantitationLimitMeasure.MeasureValue \tab numeric \tab	Constituent concentration
#' that, when processed through the complete method, produces a signal that is statistically
#' different from a blank. Measure value is given in the units stored in
#' DetectionQuantitationLimitMeasure/MeasureUnitCode.\cr
#' DetectionQuantitationLimitMeasure.MeasureUnitCode \tab character \tab	The code
